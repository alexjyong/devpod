...
fn install(_app_handle: AppHandle, force: bool) -> Result<(), InstallCLIError> {
    ...
    // /usr/local/bin/devpod-secrets
    let raw_system_bin = format!("/usr/local/bin/{}", "devpod-secrets");
    ...
    if let Some(home) = home_dir() {
        // $HOME/bin/devpod-secrets
        let mut user_bin = home.clone();
        user_bin.push("bin/devpod-secrets");

        // $HOME/.local/bin/devpod-secrets
        let mut user_local_bin = home;
        user_local_bin.push(".local/bin/devpod-secrets");
...
fn install(app_handle: AppHandle, force: bool) -> Result<(), InstallCLIError> {
    ...
    let cmd_file = BinFile {
        name: format!("{}.cmd", "devpod-secrets".to_string()),
